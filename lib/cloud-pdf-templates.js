// ============================================================
// Quanto · Custom PDF template reads (cloud)
// ============================================================
...
/**
 * Lists EVERY template id, archived rows INCLUDED. Used only to dedup a
 * new slug against the PRIMARY KEY: a soft-deleted row still owns its id,
 * so deduping against the visible gallery alone (listPdfTemplates filters
 * archived_at) would let a reused slug collide on the PK and fail with a
 * misleading generic error. This deliberately omits the archived filter.
 *
 * @param {object} client - D1 client
 * @returns {Promise<string[]>}
 */
async function listAllPdfTemplateIds(client) {
  const res = await client.query('SELECT id FROM pdf_templates');
...
module.exports = { loadPdfTemplate, listPdfTemplates, listAllPdfTemplateIds, insertPdfTemplate };
