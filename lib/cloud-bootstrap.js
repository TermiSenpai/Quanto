...
function createCloudBootstrap({
  ...
  // A network failure (offline / timeout / TLS) means the quote/status
  // must be queued, not lost. A non-network error (e.g. bad status) is a
  // client bug surfaced as { ok:false }. We treat any upload throw as an
  // outage worth queuing EXCEPT the status validation, which is checked
  // before any network call (lib/cloud-quotes.js updateQuoteStatus).
  function isOffline(err) {
    return /No se pudo conectar|no respondió a tiempo|Cloudflare/i.test(err && err.message || '');
  }
