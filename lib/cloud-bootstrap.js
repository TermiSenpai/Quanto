...
function createCloudBootstrap({
  ...
  // Bundled so the pending-migration check and the entity read share the
  // SAME 5s network cutoff (withTimeout) as one unit — a hung migration
  // check must not be allowed to hang the boot any longer than a hung
  // catalog read already can (UI-UX §2.2).
  async function readCatalogEntities(client, settings) {
    await ensurePendingMigrations(client, settings);
    return loadEntities(client);
  }

  async function fetchFromCloud(settings) {
    const client = clientFromSettings(settings);
    const { entities, meta } = await withTimeout(readCatalogEntities(client, settings));
    const fetchedAt = now();
  ...
  async function ensurePendingMigrations(client, settings) {
    ...
    const migration = await migrateWithLock(client, { user: resolveAuthor(settings) });
