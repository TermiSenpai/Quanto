  ...
  return {
    ...
    async provision({ token, accountId, user } = {}) {
      try {
        ...
        if (lockTs === null) {
          return { ok: false, code: 'MIGRATION_LOCKED', error: LOCKED_MESSAGE };
        }
        ...
        try {
          ...
          return { ok: false, code: 'MIGRATION_FAILED', error: err.message, backupPath };
        } finally {
    ...
    /** catalog:load → cloud → cache → { ok:false, code:'NO_CLOUD_NO_CACHE' } */
      ...
      try {
        ...
        if (cached) {
          ...
          return {
            ...
            catalogVersion: cached.catalogVersion, fetchedAt: cached.fetchedAt, offline: true
          };
        }
        return { ok: false, code: 'NO_CLOUD_NO_CACHE', error: cloudError.message };
      } catch (cacheErr) {
        // Corrupt cache (or cache that assembles into an invalid
        // config): surfaced, never silently treated as data.
        return { ok: false, code: 'NO_CLOUD_NO_CACHE', error: cacheErr.message };
      }
    },
...
module.exports = { createCloudBootstrap, DB_NAME };
