...
//
// The §6 migration-safety contract (migrateWithLock):
...
//   { ok:false, code:'MIGRATION_FAILED', backupPath }. cloud:provision
//   runs it once at first boot; ensurePendingMigrations runs it again at
//   the START of every later cloud read (fetchFromCloud) so an EXISTING
//   customer database picks up whatever migrations a newer .exe bundles
//   — never only at the one-time provision.
//
 ...
 * @param {number} [deps.migrationTimeoutMs] - SEPARATE, much larger cutoff for
 *   the §6 migration-safety sequence (lock+backup+apply+verify) run before
 *   every cloud read; default 120000 ms. It must be its own budget, not the
 *   5s read cutoff: lib/d1-client.js polls exportDatabase() up to 60x1s, and
 *   verify does a full loadEntities+validate round-trip — routinely far
 *   longer than a plain catalog read.
 * @param {(msg: string, meta?: object) => void} [deps.log] - sink for dropped cloud errors (never silently swallowed)
 * @param {(client: object, quote: object) => Promise<any>} [deps.drainFullQuote] -
 *   re-saves a full reopenable quote buffered offline (Phase B fullQuotes
 *   lane). main injects it because the drain owns cache reconciliation +
 *   provisional-id assignment (main-side concerns); when absent the lane is
 *   preserved for a later flush.
 */
function createCloudBootstrap({
  ...
  // Boot must never hang on a half-dead connection: a read is raced
  // against a single timer (UI-UX §2.2 — 5 s cutoff by default). `ms`
  // lets a caller opt into a DIFFERENT budget for a longer operation —
  // the migration sequence gets its own, much larger one
  // (migrationTimeoutMs) — without duplicating the race/cleanup.
  function withTimeout(promise, ms = timeoutMs) {
    let timer;
    const cutoff = new Promise((_resolve, reject) => {
      timer = setTimeout(() => reject(new Error(`Cloudflare no respondió a tiempo (${ms} ms)`)), ms);
    });
  ...
  async function fetchFromCloud(settings) {
    ...
    // ensurePendingMigrations manages its OWN timeouts internally (a cheap
    // 5s-budget existence/ledger gate, and — only once real migration work
    // is confirmed necessary — the separate, much larger migrationTimeoutMs
    // budget). It is deliberately NOT wrapped in another withTimeout here:
    // that would force the cheap gate to share a 120s cutoff too, so a
    // totally-unreachable cloud (nothing pending, nothing to migrate) would
    // take 120s instead of 5s to fail fast into the ordinary cache fallback.
    const preloaded = await ensurePendingMigrations(client, settings);
    // A migrating boot's verifyCatalog (inside migrateWithLock) already
    // loaded the full catalog — reuse it instead of reading everything a
    // second time. Only when nothing was preloaded (no pending migration,
    // or one skipped because the catalog was still empty) do we read here,
    // under the ordinary 5 s read cutoff.
    const { entities, meta } = preloaded || await withTimeout(loadEntities(client));
    const fetchedAt = now();
  ...
  async function flushOutboxBestEffort(settings, client) {
    ...
    try {
      const deps = { uploadQuote, updateQuoteStatus, now };
      // The Phase B fullQuotes lane drains through main's injected callback
      // (it reconciles a provisional offline-create id into a real one and
      // refreshes the per-PC cache). Bind the live client so it shares the
      // proven connection. When not injected the lane is simply preserved.
      if (typeof drainFullQuote === 'function') {
        deps.drainFullQuote = (quote) => drainFullQuote(client, quote);
      }
      const res = await flushOutbox(userDataDir, client, deps);
      if (res.uploaded || res.statusesApplied || res.fullQuotesDrained || res.remaining || res.error) {
  ...
  // Post-migration sanity check (§6 «VERIFICAR»): only when the catalog
  // already has data — a fresh empty base has nothing to assemble yet
  // (its content arrives with the seed right after). Returns the
  // { entities, meta } it loaded, so a migrating boot can reuse it
  // instead of loading the whole catalog a second time — or null when it
  // skipped (nothing to verify, nothing to reuse either).
  async function verifyCatalog(client) {
    const probe = await client.query('SELECT COUNT(*) AS n FROM products');
    if (((probe.results || [])[0] || {}).n > 0) {
      const loaded = await loadEntities(client);
      toValidatedConfig(loaded.entities, now());
      return loaded;
    }
    return null;
  }

  // The §6 migration-safety contract itself, shared by cloud:provision
  // (first boot) and ensurePendingMigrations (every later cloud load, once
  // a newer .exe bundles migrations an already-provisioned database
  // hasn't seen yet): CANDADO → BACKUP (only when there is something to
  // protect) → MIGRAR → VERIFICAR → release the lock in `finally`, even on
  // failure (a throw while releasing must never replace the in-flight
  // result — swallow + log instead).
  async function migrateWithLock(client, { user }) {
    ...
    try {
      ...
      const applied = await applyMigrations(client, loadMigrations(), { user, appVersion, now });
      // `loaded` (the catalog verifyCatalog just read) rides along so a
      // migrating boot can reuse it instead of a second loadEntities.
      const loaded = await verifyCatalog(client);
      return { ok: true, applied, loaded };
    } catch (err) {
      ...
      try {
        ...
        log('migration lock release failed (non-fatal)', { error: releaseErr.message });
      }
    }
  }

  // Applies any bundled migration an already-provisioned database hasn't
  // seen yet (planes/v5-cloud-sync.md §6): a NEW .exe against an EXISTING
  // customer base (e.g. one that only ever saw 0001+0002) must bring the
  // schema current on its own, not just at the one-time cloud:provision —
  // otherwise reads that depend on a newer table/column (0003_quote_deposits
  // is the first) fail forever until someone manually re-provisions. A
  // brand-new, unprovisioned database (no schema_migrations table at all)
  // is provision's job, not this one, so it is left untouched here.
  //
  // TWO separate timeout budgets, on purpose:
  //   - the existence check + ledger read below are two cheap SELECTs —
  //     bounded by the ordinary 5s read cutoff (withTimeout's default ms),
  //     same as any other probe, so a totally-unreachable cloud with
  //     NOTHING pending still fails fast into the normal offline/cache
  //     fallback (never mistaken for a migration failure).
  //   - migrateWithLock (the real lock+backup+apply+verify work, only run
  //     once something is actually confirmed pending) gets its OWN, much
  //     larger migrationTimeoutMs budget (default 120s) — export alone can
  //     poll for up to a minute (lib/d1-client.js) and verify does a full
  //     loadEntities round-trip, both routinely longer than 5s.
  //
  // Returns the { entities, meta } verifyCatalog loaded while migrating
  // (fetchFromCloud reuses it instead of loading twice), or null when
  // nothing was applied (nothing pending, or the lock was held elsewhere).
  async function ensurePendingMigrations(client, settings) {
    if (!(await withTimeout(tableExists(client, 'schema_migrations')))) return null;
    // This ledger read, BEFORE taking the lock, is only a cheap gate to
    // skip the lock entirely when nothing is pending — it is not the
    // authoritative check. applyMigrations re-reads the ledger again,
    // for real, once the lock is actually held.
    const res = await withTimeout(client.query('SELECT id FROM schema_migrations'));
    const appliedIds = new Set((res.results || []).map((row) => row.id));
    const pending = loadMigrations().filter((m) => !appliedIds.has(m.id)).map((m) => m.id);
    if (pending.length === 0) return null;

    log('catalog:load applying pending migrations', { pending });
    let migration;
    try {
      migration = await withTimeout(migrateWithLock(client, { user: resolveAuthor(settings) }), migrationTimeoutMs);
    } catch (timeoutErr) {
      // migrateWithLock itself never REJECTS (it always resolves to an
      // { ok, ... } result, even on failure) — the only way this race can
      // reject is the migrationTimeoutMs cutoff firing while the migration
      // keeps running in the background, lock still held. Surface it as
      // MIGRATION_FAILED, never as an ordinary offline/timeout: silently
      // falling back to a stale cache over a possibly half-migrated schema
      // is exactly what §6 step 5 forbids. backupPath is NOT known here —
      // migrateWithLock never got the chance to return it.
      log('catalog:load pending migration timed out', { error: timeoutErr.message });
      const err = new Error('La actualización de la base de datos no terminó a tiempo; vuelve a intentarlo.');
      err.migrationFailed = true;
      err.code = CODES.MIGRATION_FAILED;
      throw err;
    }
    if (migration.ok) return migration.loaded || null;
    if (migration.code === CODES.MIGRATION_LOCKED) {
      // Another PC is migrating right now: proceed with the load as-is
      // (this PC just reads against the pre-migration schema for now) —
      // the next load, from any PC, will see the ledger complete.
      log('catalog:load found the migration lock held by another PC, continuing', { error: migration.error });
      return null;
    }
    // MIGRATION_FAILED: a half-migrated schema must be surfaced, never
    // silently papered over with a stale cache (planes/v5-cloud-sync.md §6
    // step 5) — tag the error so fetchFromCloud's callers (loadCatalog,
    // refreshCatalog) can tell it apart from an ordinary offline/
    // invalid-catalog failure and skip the cache fallback entirely.
    log('catalog:load pending migration failed', { error: migration.error, backupPath: migration.backupPath });
    const err = new Error(migration.error);
    err.migrationFailed = true;
    err.code = CODES.MIGRATION_FAILED;
    err.backupPath = migration.backupPath;
    throw err;
  }

  return {
    /**
     * Builds a D1 client from per-PC settings (the SAME credential mapping
     * the internal cloud ops use). Exposed so main.js's quote-repo selector
     * can obtain a client for lib/quote-repo-cloud.js without duplicating the
     * token/account/database mapping. The client holds the API token — it
     * lives only in main and never reaches the renderer.
     *
     * @param {object} settings - per-PC settings (carries `cloud`)
     * @returns {object} a lib/d1-client.js client
     */
    clientFor(settings) {
      return clientFromSettings(settings);
    },
    ...
    async provision({ token, accountId, user } = {}) {
      try {
        ...
        const migration = await migrateWithLock(client, { user });
    ...
    async loadCatalog(settings) {
      ...
      try {
        ...
        // A half-migrated schema (planes/v5-cloud-sync.md §6 step 5) must
        // be surfaced honestly, never papered over with a stale cache —
        // return immediately, skipping the cache-fallback ladder below.
        if (err.migrationFailed) {
          return { ok: false, code: CODES.MIGRATION_FAILED, error: err.message, backupPath: err.backupPath };
        }
        cloudError = err;
      }
    ...
    async refreshCatalog(settings) {
      try {
        ...
        // Same early exit as loadCatalog: a half-migrated schema is
        // surfaced as-is, never masked by the (here nonexistent) cache
    ...
    async saveCatalog(settings, { newCfg, expectedVersions, user }) {
      ...
      // Defaulted like the config the renderer received, so a legacy
      // catalog never diffs a phantom quote_settings change; the key is
      // persisted when Empresa is really edited.
      const oldCfg = applyQuoteSettingsDefaults(assemble(entities, {
     ...
     *
     * Legacy mirror path — no longer called by the renderer in Phase B
     * (the shared cloud backend is written directly by lib/quote-repo-cloud.js).
     * Kept pending removal. On a network failure the quote is queued
     * (lib/quote-outbox.js) and drained on the next successful sync —
     * never lost, never duplicated (UUID PK + INSERT OR IGNORE).
