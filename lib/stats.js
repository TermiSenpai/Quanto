// ============================================================
// PackPrice · Pure statistics aggregator
// ============================================================
...
function computeStats({ quotes = [], items = [], addons = [] }, cfg = {}) {
  ...
  const packs = new Map();     // packId → { count, units, total, pending, accepted, rejected, marginSum, marginCount, q3xl, q4xl, q5xl }
  ...
  for (const q of quotes) {
    ...
    pAcc.marginSum += num(q.margin_pct);
    pAcc.marginCount++;
  ...
  for (const [packId, a] of packs) {
    ...
    marginByPack.push({
      packId, name,
      realMarginPct: safeDiv(a.marginSum, a.marginCount),
  ...
  return {
    ...
    avgMarginPct: safeDiv(marginSum, totalCount),
    targetMarginPct,
    // breakdowns
    byPack,
    conversionByPack,
