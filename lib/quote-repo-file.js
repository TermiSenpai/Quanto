 ...
 * updateQuote, which patched status without bumping version): `version` is
 * NOT bumped, so the cloud backend's version token stays valid; in file
 * mode the token is a content hash, so this write DOES invalidate an open
 * editor's token — a later edit-save gets a (spurious) conflict dialog,
 * resolvable with Sobrescribir, and the pinned workflow fields survive the
 * overwrite.
 *
 ...
 * 'pending' (design 2026-09-04 §4.1). Like setStatus this is workflow, not
 * a content edit: `version` is NOT bumped, so the cloud backend's version
 * token stays valid; in file mode the token is a content hash, so this
 * write DOES invalidate an open editor's token — a later edit-save gets a
 * (spurious) conflict dialog, resolvable with Sobrescribir, and the pinned
 * workflow fields survive the overwrite. The payment is validated before
 * any IO.
 *
 * @param {string} folder
 * @param {string} id
