// ============================================================
// Quanto · Strict config schema validator (v4)
// ============================================================
...
// ------------------------------------------------------------
// Hex color: #RGB or #RRGGBB. Used for the optional brand color (the
// PDF template tint). Validated only when present (additive, optional).
...
function validateCompany(company, errors) {
  ...
  // Optional shared PDF settings (v5, Plan 6): the selected template id
  // and the brand color. Both optional; validate only when present.
  if (company.pdf_template !== undefined && company.pdf_template !== null
      && typeof company.pdf_template !== 'string') {
    errors.push('"company.pdf_template" debe ser un texto (id de plantilla).');
  }
  if (company.brand_color !== undefined && company.brand_color !== null && company.brand_color !== '') {
    if (typeof company.brand_color !== 'string' || !HEX_COLOR_RE.test(company.brand_color)) {
      errors.push(`"company.brand_color" debe ser un color hex (#RGB o #RRGGBB) (recibido: ${describe(company.brand_color)}).`);
    }
  }
}
...
function validateQuoteSettings(qs, errors) {
  ...
  // null = not set (a D1 row may carry JSON null); applyQuoteSettingsDefaults fills it.
