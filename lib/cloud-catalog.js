// ============================================================
// Quanto · Cloud catalog read path + initial seed
// ============================================================
...
async function loadEntities(client) {
  // Meta FIRST, entities after: if a write lands while we are still
  // selecting tables, the version we cache is the *older* one, so the
  // next freshness probe sees a mismatch and refetches. Reading meta
  // last would let catalogVersion overstate how fresh the rows are
  // and the stale snapshot would stick until the next real write.
...
async function seedCatalog(client, entities, { user, now }) {
  ...
  // Bump catalog_version so any cached pre-seed (empty) snapshot is
  // invalidated by the next freshness probe.
  await client.query(
...
module.exports = { ENTITY_TABLES, ARCHIVABLE_TABLES, loadEntities, getCatalogVersion, seedCatalog };
