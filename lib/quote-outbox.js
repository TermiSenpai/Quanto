...
// them on reconnect (planes/v5-cloud-sync.md §7). The shared quote store
// (file folder / D1 quote_payloads) is the source of truth (Phase B);
// this outbox is the offline write buffer for the shared store — a
// temporary outage never loses a quote and the next successful sync
// uploads it.
//
 ...
 */
// Removes the FIRST entry of `list` that `match(entry)` returns true for,
// mutating `list` in place. Used at the commit point to subtract exactly
// the drained items from a FRESH read of the queue (so a concurrent
// enqueue during an await is preserved, not clobbered). Removing only the
// first match means a duplicate appended mid-flush survives — it is a
// genuine new request, not the one we just drained.
function removeFirst(list, match) {
...
async function flushOutbox(userDataDir, client, deps) {
  ...
  // Record the entries actually drained this pass (NOT the kept ones): at
  // the commit point we re-read the queue and subtract exactly these, so an
  // enqueue that landed during an await is never overwritten (lost-update
  // race — the primary offline-save path writes this lane and the flush
  // fires on every catalog load/refresh).
  const drainedQuoteIds = [];
  const drainedStatuses = [];
  const drainedFullIds = [];
  let uploaded = 0;
  ...
  if (typeof deps.drainFullQuote === 'function') {
    for (const quote of outbox.fullQuotes) {
      try {
        ...
        fullQuotesDrained++;
        // Match by the QUEUED id (a create still carries its provisional id
        // here; the drain assigns the real id only downstream).
        drainedFullIds.push(quote.id);
      } catch (_) { /* stays queued */ }
    }
  }

  // Commit point: re-read the LIVE queue and subtract only what we drained,
  // preserving anything enqueued during the awaits above.
  let fresh;
  try {
    fresh = readOutbox(userDataDir);
  } catch (err) {
    // The queue went corrupt mid-flush: surface it rather than overwrite a
    // file we can no longer reason about (don't risk clobbering live data).
    return { uploaded, statusesApplied, fullQuotesDrained, remaining: 0, error: err.message };
  }

  for (const id of drainedQuoteIds) removeFirst(fresh.quotes, (q) => q.id === id);
  for (const s of drainedStatuses) {
    removeFirst(fresh.statuses, (e) => e.id === s.id && e.status === s.status && e.ts === s.ts);
  }
  for (const id of drainedFullIds) removeFirst(fresh.fullQuotes, (q) => q.id === id);

  writeOutbox(userDataDir, fresh);
  return {
    uploaded,
    statusesApplied,
    fullQuotesDrained,
    remaining: fresh.quotes.length + fresh.statuses.length + fresh.fullQuotes.length
  };
}
