 ...
 *
 * Minimal shape check: the payload must be a non-null object with a string
 * `fetchedAt` (the staleness key) and a `list` array. A missing `payloads`
 * field is tolerated (forward-compat), but a present-but-wrong-typed one is
 * corruption.
 *
 * @param {string} userDataDir
...
function readQuoteCache(userDataDir) {
  ...
  // Minimal shape check: a non-null object with a string `fetchedAt` (the
  // staleness key) and a `list` array. `payloads` is optional — an ABSENT
  // key is tolerated (forward-compat), but a present-but-wrong-typed one
  // (number, string, array) is corruption: a consumer doing
  // `cache.payloads[id]` would read garbage, the same class of bug the
  // non-array-`list` hard-fail catches. So tolerate undefined, else require
