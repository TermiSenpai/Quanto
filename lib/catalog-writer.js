...
async function writeEntities(client, { oldCfg, newCfg, user, now, expectedVersions }) {
  ...
  for (const { entityType, id } of changed) {
    if (PER_ID[entityType]) {
      ...
      // version-guard against. I2: two PCs creating the SAME new id would
      // otherwise both INSERT and the second hits a PK violation that
      // escapes as a raw throw. Check existence first and convert the
      // collision into a conflict (R18), so the partial-success machinery
      // and the renderer's conflict UX handle it like any other conflict.
      if (expected === undefined) {
        const existing = await client.query(
          `SELECT ${spec.idColumn} FROM ${spec.table} WHERE ${spec.idColumn} = ?`,
          [id]
        );
