...
/**
 * Reads all quotes. Returns [] when the file does not exist.
 * Throws on parse error so callers can surface the issue (we do
 * NOT silently ignore corruption — that would mask data loss).
 */
...
function listQuotes(userDataDir) {
  const all = readAllQuotes(userDataDir);
  return all.slice().sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''));
}

/**
 * Substring search across id, cliente.nombre, and usuario. Case
