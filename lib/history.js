...
/**
 * Replaces an existing quote in place (edit flow). The draft's pricing
 * inputs, result, customer, and metadata fields are merged in, but the
 * original `id`, `date`, and workflow fields (`status`, `status_ts`,
 * `cloud_id`) are preserved — editing changes WHAT was priced, not the
 * accept/reject state or the cloud link. Returns null when the id is
 * unknown (no write). Version is bumped and updated_at is refreshed.
 *
 * @param {string} userDataDir
 * @param {string} id
 * @param {object} draft  the updated quote payload
 * @param {object} [opts]
 * @param {Date}   [opts.now]  for tests
 * @returns {object|null} the merged entry, or null if id not found
 */
function replaceQuote(userDataDir, id, draft, opts = {}) {
  if (!draft || typeof draft !== 'object' || Array.isArray(draft)) {
    throw new Error('El presupuesto debe ser un objeto.');
  }
  const all = readAllQuotes(userDataDir);
  ...
  const merged = {
    ...draft,
    // Pin the identity and workflow fields from the existing entry.
    id: existing.id,
    date: existing.date,
    status: existing.status,
    status_ts: existing.status_ts,
    cloud_id: existing.cloud_id,
    // Bump the revision counter and timestamp.
    version: (Number.isFinite(existing.version) ? existing.version : 1) + 1,
    updated_at: now.toISOString()
  };
  // Remove undefined workflow fields so the merged object stays clean
  // (if the original never had status_ts/cloud_id, don't add undefined keys).
  // Status is special: a legacy entry may lack it, but the draft always
  // carries one ('pending') — keep the draft's value rather than dropping it.
  if (merged.status === undefined) {
    if (draft.status !== undefined) merged.status = draft.status; // legacy entry had none: keep the draft's
  ...
  // Bound the serialized size of the merged entry (mirrors saveQuote's cap).
  let serialized;
  try {
    serialized = JSON.stringify(merged);
  } catch (_) {
