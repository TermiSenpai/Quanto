 ...
 * id/date/user/customer.name/pack/total/status/deposit_paid alone.
 *
 * @param {object} client - D1 client
...
async function listQuotes(client) {
  ...
  const list = rows.map((row) => ({
    ...
    status: row.status,
    pack_id: row.pack_id,
    deposit_paid: depositFromRow(row),
  }));
 ...
 * the stored record and PRESERVES status/status_ts/deposit_paid (edits don't
 * change workflow state — mirrors lib/quote-repo-file.js replaceQuote and
 * lib/history.js); sets updated_at to now; takes the rest from the draft. The
 * `deposit_paid` copy written into the payload is decorative — `getFullQuote`'s
 * flat-row overlay always wins on read.
 *
 ...
 * payment is validated BEFORE any network call; the flat row + deposit row
 * are written by lib/cloud-quotes.js setQuoteDeposit; the payload version
 * is untouched (the version token of an open editor stays valid).
 *
 * @param {object} client - D1 client
 * @param {string} id
