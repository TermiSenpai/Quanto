...
// quote_payloads — 0002; quote_deposits — 0003) over the injected D1
// client. Quotes are historical records, so
 ...
 * @returns {Promise<{ok: boolean, id: string, status: string, changes: number}>}
 *   `changes` is the number of flat rows updated (0 when the id is unknown),
 *   so a caller honoring an updated|null contract can detect a not-found.
 */
async function updateQuoteStatus(client, { id, status, now }) {
  if (!QUOTE_STATUSES.includes(status)) {
    throw new Error(`Estado de presupuesto no válido: ${status}`);
  }
  const res = await client.query('UPDATE quotes SET status = ?, status_ts = ? WHERE id = ?', [status, now, id]);
  const changes = (res && res.meta && res.meta.changes) || 0;
  return { ok: true, id, status, changes };
}
 ...
 * with the authoritative status/status_ts/deposit_paid overlaid from the
 * flat `quotes` row LEFT JOINed to `quote_deposits` (the flat row is the
 * source of truth for status/deposit — it is what updateQuoteStatus and
 * setQuoteDeposit mutate). Returns null when the payload row is absent. A
 * corrupt payload throws a Spanish error (no silent swallow).
 *
 * @param {object} client - D1 client
...
async function getFullQuote(client, id) {
  ...
  // ALWAYS set deposit_paid — object or null — even with no flat row (an
  // orphan payload), so a stale copy inside the payload can never win.
  quote.deposit_paid = depositFromRow(flatRow);
  return quote;
}

/**
 * Lists quotes from the flat `quotes` columns LEFT JOINed to
 * `quote_deposits` — the list view needs id, user, client_name, pack_id,
 * total_vat_inc, status, ts and the paid-deposit columns (null when
 * unpaid), never the full payload. `user` and `pack_id` feed the
 * renderer's history list (lib/quote-repo-cloud.js maps these flat
 * columns to its list-row shape so the list renders without a per-quote
 * payload fetch).
 *
 * @param {object} client - D1 client
 ...
 * foreign keys (quote_deposits / quote_items / quote_addons reference
 * quotes(id)), then the flat `quotes` row, then the `quote_payloads` row.
 * All DELETEs are parameterized on the id. The façade
 * (lib/quote-repo-cloud.js) reads the row BEFORE calling this so it can
 * return the removed quote — this op only needs to report that it ran.
 *
 * @param {object} client - D1 client
 * @param {string} id - the human quote id
